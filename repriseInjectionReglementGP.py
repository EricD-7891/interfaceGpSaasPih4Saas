...
def initialiseParametreGlobal () :
    ...
    NomFicTrace = 'C:\\Exploitation\\interfacesGpPih4\\Trace\\repriseReglements_'+datetime.now().strftime("%d-%m-%Y-%H%M%S")+'.txt'
    NomFicDonnees = 'C:\\Exploitation\\interfacesGpPih4\\Trace\\repriseReglements_'+datetime.now().strftime("%d-%m-%Y-%H%M%S")+'.csv'
    if DoTrace : 
        globalFicTrace = open (NomFicTrace,'x')
        globalFicDonnees = open (NomFicDonnees,'x')
...
def ListeFactures (numeroOperation) :
    try :
        lArgument = f'table=factures&champ=factures.Numero_operation&val={numeroOperation}&prop=passation,code_facture,numero_operation,facture_ttc,contrepartie,reglee,Reference_Facture,Date_export_comptable'
        leTest = requeteGetGP (lArgument)
        if (leTest is None) :
            return ({})
        if (len(leTest.text) == 0 ) :
            ...
            trace (f'Origine : {len(laListe)} factures')
            leDicoFactures = []
            for FactGp in laListe :
                dateExportComptable = FactGp['Date_export_comptable']
                if dateExportComptable is not None :
                    dateExport = int(dateExportComptable[0:4]+dateExportComptable[5:7]+dateExportComptable[8:10])
                    if (         FactGp['Passation']== globalAcheteurGP  and FactGp["Contrepartie"][0:3] in ('401','404','402') 
                         ...
                         and dateExport > 20241126 and dateExport < 20250318) :
                       if not (testExistanceDecaissement (FactGp["Code_facture"])) :
                           leDicoFactures.append(FactGp)
            trace (f'Prepare : {len(leDicoFactures)} factures a traiter')
            return (leDicoFactures)
    except :
        trace (f'Exception Raise on ListeFactures')
        return ({})       

def listePiecesPotentiellesPIH () : # Attention, l'order by a du sens !! A030 01
    requeteSqlPih = "select distinct a.opcctr OPCCTR, a.opphas OPPHAS  from kregatt a  where a.scso = '"+globalScsoPih+ "' \
                    and a.opcctr is not null and a.opphas is not null and a.regval = 'O' and a.letdate > add_months (sysdate, -6) \
                    and a.nopie > 0 and a.nopie < 100000 and a.faexe >= to_number(to_char(sysdate,'YYYY')) - 1 \
                    and a.ori_tjcod = 'DI' and a.fadpai < to_date ('20250318','YYYYMMDD') and a.fadpai > to_date ('20241126','YYYYMMDD') \
    ...
    trace (f'Nb operation candidate = {len(listeReglementPih)}')
...
def doIt ():
    ...
    if initialiseConnexionBasePih () :
        listeOpPIH = listePiecesPotentiellesPIH ()
        oldOpcctr = 'SITAPA2SOU'
        oldOpphas = 'TKBOIR2LO'
        oldOnSenFou = 'PASDUWISKI'
        listeOperation = ListeProjet ()
        closeConnexionBasePih ()
        for opPIH in listeOpPIH :
            globalNbParcouru += 1
            trace (f'Traitement de operation {opPIH}')
            if opPIH ["OPCCTR"]+opPIH["OPPHAS"] in listeOperation :
                if (oldOpcctr == opPIH ["OPCCTR"] and oldOpphas == opPIH ['OPPHAS']) :
                    pass 
                else : 
                    numeroOpGP=listeOperation[opPIH ["OPCCTR"]+opPIH["OPPHAS"]]
                    DicoFactureGP = ListeFactures (numeroOpGP) 
                    trace (f'Pour operation {numeroOpGP} il y a {len(DicoFactureGP)} factures candidates')
#                    DicoRecetteGP = ListeRecettes (numeroOpGP)
                    oldOpcctr = opPIH ["OPCCTR"] 
                    oldOpphas = opPIH ['OPPHAS']
                for factGP in DicoFactureGP :
                    NewDecaissement =  {'Nom_Table': 'Decaissement', 
                                        'Valeur_ID': 0, 
                                        'Numero': 1, 
                                        'Decaissement_TTC': factGP['Facture_TTC'], 
                                        'Date_decaissement': factGP['Date_export_comptable'][0:10], 
                                        'Code_facture': factGP["Code_facture"], 
                                        'Numero_operation': factGP["Numero_operation"], 
                                        'Export_comptable': True, 
                                        'Date_export_comptable':  factGP['Date_export_comptable'][0:10], 
                                        'Type_de_reglement': 'Autre', 
                                        'Date_emission_decaissement':  factGP['Date_export_comptable'][0:10],
                                        'Reference_decaissement': '###-'+strip_accents(factGP["Reference_Facture"]),
                                        'Export_Etebac': False, 
                                        'Date_de_saisie':  factGP['Date_export_comptable'][0:10],
                                        ...
                                        'Commentaires': ''}
                    globalFicDonnees.write (f'{opPIH ["OPCCTR"]};{opPIH ["OPPHAS"]};{NewDecaissement['Numero_operation']}; {NewDecaissement['Date_decaissement']}; {NewDecaissement['Journal']}; {NewDecaissement['Reference_decaissement']}; {NewDecaissement['Decaissement_TTC']};')
                    globalFicDonnees.write("\n")
